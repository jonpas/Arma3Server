...
async fn main() -> Result<()> {
    ...
    // No Steam session at all when SKIP_INSTALL is set and no mod preset
    // is configured (see Config::from_env) -- there is nothing left to
    // download, so we go straight from here to launching whatever is
    // already on disk.
    let pool = match &cfg.steam_auth {
        Some(auth) => {
            info!("Logging in to Steam ({CONNECTION_POOL_SIZE} connections)...");
            let pool = CmPool::start(CONNECTION_POOL_SIZE, auth, Path::new(SERVER_ROOT)).await?;
            info!("Logged in to Steam ({CONNECTION_POOL_SIZE} connections)");
            Some(pool)
        }
        None => {
            info!("SKIP_INSTALL is set and no mod preset is configured, skipping Steam login");
            None
        }
    };
    ...
    if pool.is_some() {
        info!(
            "Logged in to Steam in {:.1}s",
            (authed_at - process_start).as_secs_f64()
        );
    }

    // Only meaningful when there is a Steam session; stays equal to
    // authed_at on the no-login path so launch::run's timing breakdown
    // reports zero login/resolution/download time instead of garbage.
    let mut resolved_at = authed_at;
    ...
    if let Some(pool) = &pool {
        ...
        let server_fut = async {
            // v2's `if os.environ["SKIP_INSTALL"] in ["", "false"]` guard,
            // which covered exactly the base/Linux-server/profiling depots
            // and the CDLC depots -- workshop mods below were always
            // independent of it and still are.
            if cfg.skip_install {
                info!("SKIP_INSTALL is set, skipping Arma 3 server and CDLC install");
